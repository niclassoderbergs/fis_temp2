...
import { content804Input } from './content-domain-8';
...
const diagramCode = `sequenceDiagram
    ...
    SP->>CUMA: 15.1 Request deregistration (ID: A)
    activate CUMA
    CUMA->>CUMA: Validate (No active contracts)
    
    alt Validation OK
        CUMA->>CUMA: Terminate Account
        CUMA-->>SP: 15.2 Confirm deregistration
    else Active Contracts
        CUMA-->>SP: Error (Clean up portfolio first)
    end
    deactivate CUMA`;

const steps = [
  { step: "15.1", action: "Request deregistration", description: "SP requests to leave the market.", producer: "SP", receiver: "CUMA", infoId: "A" },
  { step: "15.2", action: "Confirm", description: "Account closed.", producer: "CUMA", receiver: "SP", infoId: "B" }
];

const attributes = [
  { name: "SP identifier", desc: "ID." },
  { name: "End date", desc: "When." }
];

const jwgToBrsMapping: Record<string, string> = {
  "SP identifier": "SP-ID",
  "End date": "Önskat Avslutsdatum"
};

interface Props { onBack: () => void; onNavigateToBRS: (id: string) => void; }

export const JWGProcedure15: React.FC<Props> = ({ onBack, onNavigateToBRS }) => {
  const getBrsAttribute = (jwgAttrName: string) => {
    const mappedName = jwgToBrsMapping[jwgAttrName];
    if (!mappedName) return null;
    return content804Input.attributes.find(a => a.attribute === mappedName);
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.subHeader}>Frivilligt utträde.</p>

      <div style={styles.brsBox}>
        <div>
            <div style={{fontWeight: 600, fontSize: '0.85rem', textTransform: 'uppercase', marginBottom: '4px', opacity: 0.8}}>Implementerad via</div>
            <div style={styles.brsLink} onClick={() => onNavigateToBRS(brsFlex804.id)}>{brsFlex804.id}: {brsFlex804.title}</div>
        </div>
      ...
      <section>
        ...
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>JWG Attribut</th><th style={styles.th}>Motsvarighet i {brsFlex804.id}</th></tr></thead>
          <tbody>
            {attributes.map((a, i) => {
      ...
      <section>
        ...
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex804.id} (InfoObject: {content804Input.title}). Här visas vilka JWG-krav som attributet uppfyller.</p>
        <table style={styles.table}>
          ...
          <tbody>
            {content804Input.attributes.map((attr, i) => {
