...
import { content114Input, content124Input } from './content-domain-1';
...
const diagramCode = `sequenceDiagram
    ...
    SO->>SMA: Info Item BN: Request suspension
    activate SMA
    ...
    alt Validation Failed
        ...
        SMA-->>SP: Info Item BO: Notification
...
export const JWGProcedure26: React.FC<Props> = ({ onBack, onNavigateToBRS, onNavigateToProcedure }) => {
  const getBrsAttribute = (jwgAttrName: string) => {
    ...
    // Check both 114 and 124 inputs
    let attr = content114Input.attributes.find(a => a.attribute === mappedName);
    if (!attr) attr = content124Input.attributes.find(a => a.attribute === mappedName);
    ...
    <div style={styles.container}>
      ...
      <div style={styles.brsBox}>
        <div>
            ...
            <div style={styles.brsLink} onClick={() => onNavigateToBRS(brsFlex114.id)}>{brsFlex114.id}: {brsFlex114.title} (SPU)</div>
            <div style={styles.brsLink} onClick={() => onNavigateToBRS(brsFlex124.id)}>{brsFlex124.id}: {brsFlex124.title} (SPG)</div>
        </div>
      ...
      <section>
        ...
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>JWG Attribut</th><th style={styles.th}>Motsvarighet i {brsFlex114.id} / {brsFlex124.id}</th></tr></thead>
          <tbody>
            {attributes.map((a, i) => {
      ...
      <section>
        <h2 style={styles.sectionHeader}>Datainnehåll BRS (SPU - {brsFlex114.id})</h2>
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex114.id}.</p>
        <table style={styles.table}>
      ...
      <section>
        <h2 style={styles.sectionHeader}>Datainnehåll BRS (SPG - {brsFlex124.id})</h2>
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex124.id}.</p>
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>Attribut</th><th style={styles.th}>Beskrivning</th><th style={{...styles.th, backgroundColor: '#e6effc', color: '#0052cc', borderBottom: '2px solid #b3d4ff'}}>JWG Referens</th></tr></thead>
          <tbody>
            {content124Input.attributes.map((attr, i) => {
              const jwgRef = getJwgReference(attr.attribute);
              return (
                <tr key={i} style={i % 2 !== 0 ? { backgroundColor: '#f9f9f9' } : {}}>
                  <td style={styles.td}><strong>{attr.attribute}</strong></td>
