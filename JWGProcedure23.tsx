...
import { content112Input } from './content-domain-1';
...
const diagramCode = `sequenceDiagram
    title Procedure 23: Update SPU/SPG
    participant SP as SP
    participant CUMA as FIS

    SP->>CUMA: 23.1 Update Request (ID: A)
    activate CUMA
    CUMA->>CUMA: Validate
    CUMA-->>SP: 23.2 Confirm (ID: B)
    deactivate CUMA`;

const steps = [
  { step: "23.1", action: "Update", description: "Change name or other non-structural attributes.", producer: "SP", receiver: "CUMA", infoId: "A" },
  { step: "23.2", action: "Confirm", description: "Ack.", producer: "CUMA", receiver: "SP", infoId: "B" }
];

const attributes = [
  { name: "ID", desc: "Identifier." },
  { name: "New Values", desc: "e.g. Name." }
];

const jwgToBrsMapping: Record<string, string> = {
  "ID": "SPU-ID",
  "New Values": "Nytt Namn"
};

interface Props { onBack: () => void; onNavigateToBRS: (id: string) => void; }

export const JWGProcedure23: React.FC<Props> = ({ onBack, onNavigateToBRS }) => {
  const getBrsAttribute = (jwgAttrName: string) => {
    const mappedName = jwgToBrsMapping[jwgAttrName];
    if (!mappedName) return null;
    return content112Input.attributes.find(a => a.attribute === mappedName);
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.subHeader}>Uppdatering av SPU/SPG.</p>
      ...
      <div style={styles.brsBox}>
        <div>
            ...
            <div style={styles.brsLink} onClick={() => onNavigateToBRS(brsFlex112.id)}>{brsFlex112.id}: {brsFlex112.title}</div>
            <div style={{fontSize:'0.8rem', marginTop:'4px'}}>Även BRS-FLEX-122 för SPG.</div>
        </div>
      ...
      <section>
        ...
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>JWG Attribut</th><th style={styles.th}>Motsvarighet i {brsFlex112.id}</th></tr></thead>
          <tbody>
            {attributes.map((a, i) => {
      ...
      <section>
        ...
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex112.id} (InfoObject: {content112Input.title}). Här visas vilka JWG-krav som attributet uppfyller.</p>
        <table style={styles.table}>
