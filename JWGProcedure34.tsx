...
import { content5210Output } from './content-domain-5';
...
const diagramCode = `sequenceDiagram
    title Procedure 34: Validation of baseline data
    participant FIS as FIS

    FIS->>FIS: 34.1 Retrieve Data
    FIS->>FIS: 34.2 Run Validation Rules
    FIS->>FIS: 34.3 Flag Quality (ID: A)`;

const steps = [
  { step: "34.1", action: "Validate", description: "System checks baseline quality.", producer: "FIS", receiver: "Internal", infoId: "-" },
  { step: "34.2", action: "Result", description: "Quality flag stored.", producer: "FIS", receiver: "Internal", infoId: "A" }
];

const attributes = [
  { name: "Result", desc: "Quality indicator." }
];

const jwgToBrsMapping: Record<string, string> = {
  "Result": "Kvalitetsstämpel"
};

interface Props { onBack: () => void; onNavigateToBRS: (id: string) => void; }

export const JWGProcedure34: React.FC<Props> = ({ onBack, onNavigateToBRS }) => {
  const getBrsAttribute = (jwgAttrName: string) => {
    const mappedName = jwgToBrsMapping[jwgAttrName];
    if (!mappedName) return null;
    return content5210Output.attributes.find(a => a.attribute === mappedName);
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <h1 style={styles.header}>Procedure 34: Validation of baseline data</h1>
      <p style={styles.subHeader}>Validering av beräkningsunderlag.</p>

      <div style={styles.brsBox}>
        <div>
            <div style={{fontWeight: 600, fontSize: '0.85rem', textTransform: 'uppercase', marginBottom: '4px', opacity: 0.8}}>Implementerad via</div>
            <div style={styles.brsLink} onClick={() => onNavigateToBRS(brsFlex5210.id)}>{brsFlex5210.id}: {brsFlex5210.title}</div>
        </div>
      ...
      <section>
        ...
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex5210.id} (InfoObject: {content5210Output.title}). Här visas vilka JWG-krav som attributet uppfyller.</p>
        <table style={styles.table}>
