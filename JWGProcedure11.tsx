...
import { content201Input } from './content-domain-2';
...
const diagramCode = `sequenceDiagram
    title Procedure 11: Switch of Service Provider
    participant NewSP as New SP
    participant CUMA as CU Module Administrator
    participant OldSP as Old SP

    NewSP->>CUMA: 11.1 Register new contract (ID: A)
    activate CUMA
    CUMA->>CUMA: 11.2 Validate and Terminate Old Contract
    CUMA-->>NewSP: 11.3 Confirm Registration
    CUMA->>OldSP: 11.4 Notify Termination (Procedure 9)
    deactivate CUMA`;

const steps = [
  { step: "11.1", action: "Register new contract", description: "New SP sends registration request.", producer: "New SP", receiver: "CUMA", infoId: "A" },
  { step: "11.2", action: "Process Switch", description: "CUMA detects conflict, terminates old contract (if any).", producer: "CUMA", receiver: "System", infoId: "-" },
  { step: "11.3", action: "Confirm", description: "Confirm to New SP.", producer: "CUMA", receiver: "New SP", infoId: "B" }
];

const attributes = [
  { name: "CU identification", desc: "Resource." },
  { name: "Start date", desc: "Switch date." }
];
...
export const JWGProcedure11: React.FC<Props> = ({ onBack, onNavigateToBRS }) => {
  ...
  return (
    <div style={styles.container}>
      ...
      <h1 style={styles.header}>Procedure 11: Switch of Service Provider</h1>
      <p style={styles.subHeader}>Nyteckning som innebär byte av SP.</p>

      <div style={styles.brsBox}>
        <div>
            <div style={{fontWeight: 600, fontSize: '0.85rem', textTransform: 'uppercase', marginBottom: '4px', opacity: 0.8}}>Implementerad via</div>
            <div style={styles.brsLink} onClick={() => onNavigateToBRS(brsFlex201.id)}>{brsFlex201.id}: {brsFlex201.title}</div>
        </div>
