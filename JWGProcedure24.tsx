...
import { content113Input } from './content-domain-1';
...
const diagramCode = `sequenceDiagram
    title Procedure 24: De-registration SPU/SPG
    participant SP as SP
    participant CUMA as FIS

    SP->>CUMA: 24.1 De-register (ID: A)
    activate CUMA
    CUMA->>CUMA: Validate (Must be empty)
    CUMA-->>SP: 24.2 Confirm (ID: B)
    deactivate CUMA`;

const steps = [
  { step: "24.1", action: "De-register", description: "Remove SPU/SPG.", producer: "SP", receiver: "CUMA", infoId: "A" },
  { step: "24.2", action: "Confirm", description: "Confirmed removed.", producer: "CUMA", receiver: "SP", infoId: "B" }
];
...
export const JWGProcedure24: React.FC<Props> = ({ onBack, onNavigateToBRS }) => {
  const getBrsAttribute = (jwgAttrName: string) => {
    const mappedName = jwgToBrsMapping[jwgAttrName];
    if (!mappedName) return null;
    return content113Input.attributes.find(a => a.attribute === mappedName);
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.brsBox}>
        <div>
            ...
            <div style={styles.brsLink} onClick={() => onNavigateToBRS(brsFlex113.id)}>{brsFlex113.id}: {brsFlex113.title}</div>
            <div style={{fontSize:'0.8rem', marginTop:'4px'}}>Även BRS-FLEX-123 för SPG.</div>
        </div>
      ...
      <section>
        ...
        <table style={styles.table}>
          <thead><tr><th style={styles.th}>JWG Attribut</th><th style={styles.th}>Motsvarighet i {brsFlex113.id}</th></tr></thead>
          <tbody>
            {attributes.map((a, i) => {
      ...
      <section>
        ...
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex113.id} (InfoObject: {content113Input.title}). Här visas vilka JWG-krav som attributet uppfyller.</p>
        <table style={styles.table}>
