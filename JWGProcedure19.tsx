...
import { content321Input } from './content-domain-3';
...
const diagramCode = `sequenceDiagram
    title Procedure 19: Grid pre-qualification
    participant SP as SP
    participant CUMA as FIS
    participant DSO as DSO

    SP->>CUMA: 19.1 Request grid pre-qualification (ID: A)
    activate CUMA
    CUMA->>DSO: 19.2 Forward request
    activate DSO
    DSO->>DSO: Perform grid analysis
    DSO-->>CUMA: 19.3 Result (Approved/Rejected)
    deactivate DSO
    CUMA-->>SP: 19.4 Notification of result (ID: B)
    deactivate CUMA`;

const steps = [
  { step: "19.1", action: "Request pre-qualification", description: "SP requests grid check.", producer: "SP", receiver: "CUMA", infoId: "A" },
  { step: "19.2", action: "Forward", description: "CUMA routes to DSO.", producer: "CUMA", receiver: "DSO", infoId: "-" },
  { step: "19.3", action: "Result", description: "DSO provides result.", producer: "DSO", receiver: "CUMA", infoId: "-" },
  { step: "19.4", action: "Notify", description: "SP informed.", producer: "CUMA", receiver: "SP", infoId: "B" }
];

const attributes = [
  { name: "SPU/SPG identifier", desc: "Resource to check." },
  { name: "Grid area", desc: "Implied by MP." }
];

const jwgToBrsMapping: Record<string, string> = {
  "SPU/SPG identifier": "SPU-ID / CU-ID"
};

interface Props { onBack: () => void; onNavigateToBRS: (id: string) => void; }

export const JWGProcedure19: React.FC<Props> = ({ onBack, onNavigateToBRS }) => {
  const getBrsAttribute = (jwgAttrName: string) => {
    const mappedName = jwgToBrsMapping[jwgAttrName];
    if (!mappedName) return null;
    return content321Input.attributes.find(a => a.attribute === mappedName);
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <h1 style={styles.header}>Procedure 19: Grid pre-qualification of SPU or SPG</h1>
      <p style={styles.subHeader}>Kontroll hos nätägare.</p>
      ...
      <section>
        ...
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex321.id} (InfoObject: {content321Input.title}). Här visas vilka JWG-krav som attributet uppfyller.</p>
        <table style={styles.table}>
          ...
          <tbody>
            {content321Input.attributes.map((attr, i) => {
