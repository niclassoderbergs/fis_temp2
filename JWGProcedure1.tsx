...
import { content103Output } from './content-domain-1';
...
const jwgToBrsMapping: Record<string, string> = {
  ...
  // "Locational information": "Mätpunkts-ID", // Tidigare mappning
  "Customer connection point identifier": "Mätpunkts-ID", // Ny mappning enligt instruktion
  ...
  return (
    <div style={styles.container}>
      ...
      <section>
        ...
        <p style={styles.paragraph}>Tabellen nedan visar attributen som definieras i JWG Annex, samt hur dessa mappas mot datamodellen i FIS.</p>
        <table style={styles.table}>
          <thead><tr><th style={{...styles.th, width: '30%'}}>JWG Attribut</th><th style={{...styles.th, width: '35%'}}>Beskrivning enligt JWG</th><th style={{...styles.th, width: '35%', backgroundColor: '#e3fcef', color: '#004d33', borderBottom: '2px solid #b3d4ff'}}>Motsvarighet i {brsFlex103.id}</th></tr></thead>
          <tbody>
            {attributes.map((a, i) => {
              ...
              return (
                <tr key={i} style={i % 2 !== 0 ? { backgroundColor: '#f9f9f9' } : {}}>
                  ...
                  <td style={{...styles.td, backgroundColor: i % 2 !== 0 ? '#f1fbf7' : '#fafffd'}}>
                    {brsMatch ? <><span style={styles.mappingTag}>{brsMatch.attribute}</span><div style={{fontSize: '0.8rem', color: '#555'}}>{brsMatch.description}</div></> : <span style={{fontSize: '0.8rem', color: '#999', fontStyle: 'italic'}}>-</span>}
                  </td>
                </tr>
              );
            })}
      ...
      <section>
        ...
        <p style={styles.paragraph}>Följande attribut ingår i specifikationen för {brsFlex103.id} (InfoObject: {content103Output.title}). Här visas vilka JWG-krav som attributet uppfyller.</p>
        <table style={styles.table}>
          ...
          <tbody>
            {content103Output.attributes.map((attr, i) => {
